...
class DataReader(DataController):
    ...
    def get_image_data(self, fileName):
        ...
        if os.path.isdir(fileName):
            ...
            if len(files) > 0:
                return self.get_image_data_from_dir(fileName)
            else:
        ...
        if not self.is_extension_supported(extension):
            raise Exception(extension + " is not supported.")
        imageData = self.get_image_data_for_base_and_extension(fileName, extension)
        return imageData

    def get_image_data_for_base_and_extension(self, fileName, extension):
        """
        ...
        if extension == DataReader.TypeMHA or extension == DataReader.TypeMHD:
            ...
            return self.get_image_data_from_dir(dirName)
                ...
                for z in range(int(dimensions[2])):
                    for y in range(int(dimensions[1])):
                        for x in range(int(dimensions[0])):
                            imageData.SetScalarComponentFromFloat(x, y, z, 0,
                                                                  float(fileData[dataIndex]))
                            dataIndex += 1
    ...
    def get_image_data_from_dir(self, dirName):
        """
        ...
        self.sanitize_image_data(imageReader, imageData)
        return imageData

    def sanitize_image_data(self, reader, imageData):
        """
